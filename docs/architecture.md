...
- `ConfigMap` for `bets.yaml` — generated by Kustomize from `config/bets.yaml` at the repo root, so the bet definitions live in exactly one file. Apply with `kubectl apply -k .`.
...
4. **Start builder:** `kubectl apply -k .` (from the repo root) — Kustomize generates the ConfigMap from `config/bets.yaml` and applies everything; the builder starts polling and writes `data/state.json`.
