...
- `ConfigMap` for `bets.yaml` (or bundled into the image — TBD).
- `Secret` for the results API key (referenced as env var in the Deployment).
...
4. **Start builder:** `kubectl apply -f k8s/` — builder starts polling and writes `data/state.json`.
...
| ConfigMap vs image-bundled bets.yaml? | TBD — ConfigMap preferred (no rebuild to change bets pre-tournament) |
